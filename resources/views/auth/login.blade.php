...
{{--
|--------------------------------------------------------------------------
| ExpoDKR – Page de connexion (premium redesign)
| Conserve : route('login') POST, route('password.request'), route('register')
| x-guest-layout remplacé par layout HTML standalone
|--------------------------------------------------------------------------
--}}

<div style="display:grid; min-height:100vh;" class="lg:grid-cols-2" x-data="{ showPass: false, loading: false }">

    {{-- ══════════════════════════════════════════════════════
         COLONNE GAUCHE — DÉCO (desktop seulement)
         ══════════════════════════════════════════════════════ --}}
    <div class="hidden lg:flex" style="background:var(--blue-night); position:relative; overflow:hidden; flex-direction:column; justify-content:space-between; padding:3rem;">

        {{-- Grille déco --}}
        <div class="hero-grid" style="position:absolute; inset:0; opacity:.6;" aria-hidden="true"></div>

        {{-- Glows --}}
        <div style="position:absolute; top:-10rem; left:-10rem; width:32rem; height:32rem; border-radius:50%; background:var(--blue-electric); opacity:.07; filter:blur(80px);" aria-hidden="true"></div>
        <div style="position:absolute; bottom:-8rem; right:-8rem; width:24rem; height:24rem; border-radius:50%; background:var(--gold); opacity:.06; filter:blur(60px);" aria-hidden="true"></div>

        {{-- Logo --}}
        <div style="position:relative; z-index:1;">
            <a href="{{ route('home') }}" style="display:flex; align-items:center; gap:.75rem; text-decoration:none;">
                <span class="font-display" style="font-size:1.5rem; color:white;">
        ...
        {{-- Texte central --}}
        <div style="position:relative; z-index:1;">
            <div style="display:inline-flex; align-items:center; gap:.625rem; padding:.5rem 1rem; border-radius:2rem; border:1px solid rgba(201,168,76,.3); background:rgba(201,168,76,.07); margin-bottom:2rem;">
                <span style="width:.5rem; height:.5rem; border-radius:50%; background:var(--gold); animation:pulse 2s infinite;"></span>
                <span style="font-size:.7rem; font-weight:600; letter-spacing:.15em; text-transform:uppercase; color:var(--gold-light);">Plateforme officielle · Sénégal</span>
            </div>

            <h2 class="font-display" style="font-size:2.5rem; color:white; line-height:1.2; margin-bottom:1.25rem;">
                Le rendez-vous des<br>
                <span style="background:linear-gradient(135deg,var(--gold),var(--gold-light));-webkit-background-clip:text;-webkit-text-fill-color:transparent;">
                    événements pro
                </span>
                <br>au Sénégal
            </h2>

            <p style="font-size:.9rem; color:rgba(255,255,255,.55); line-height:1.7; max-width:28rem;">
                Accédez à votre espace ExpoDKR pour gérer vos réservations, suivre vos billets et découvrir les meilleurs événements professionnels.
            </p>

            {{-- Stats --}}
            <div style="display:flex; gap:2rem; margin-top:2.5rem; padding-top:2rem; border-top:1px solid rgba(255,255,255,.08);">
                @foreach([['240+','Événements'],['180+','Exposants'],['+15k','Visiteurs']] as [$val, $lbl])
                <div>
                    <p class="font-display" style="font-size:1.75rem; color:white;">{{ $val }}</p>
                    <p style="font-size:.72rem; color:rgba(255,255,255,.4); margin-top:.15rem;">{{ $lbl }}</p>
                </div>
                @endforeach
            </div>
        </div>
        ...
        {{-- Témoignage --}}
        <div style="position:relative; z-index:1; padding:1.25rem; border-radius:1.25rem; background:rgba(255,255,255,.05); border:1px solid rgba(255,255,255,.08);">
            <div style="display:flex; gap:.25rem; margin-bottom:.75rem;">
                @for($i=0;$i<5;$i++)
                <svg style="width:.9rem;height:.9rem;color:var(--gold);" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                @endfor
            </div>
            <p style="font-size:.82rem; color:rgba(255,255,255,.65); line-height:1.6; font-style:italic;">
                "ExpoDKR a transformé la façon dont nous gérons nos participations aux salons. Interface intuitive et visibilité inégalée."
            </p>
            <div style="display:flex; align-items:center; gap:.75rem; margin-top:.875rem;">
                <div style="width:2rem; height:2rem; border-radius:50%; background:linear-gradient(135deg,var(--blue-electric),var(--gold)); display:flex; align-items:center; justify-content:center; font-size:.75rem; font-weight:700; color:white; flex-shrink:0;">A</div>
                <div>
                    <p style="font-size:.78rem; font-weight:600; color:white;">Aminata Diallo</p>
                    <p style="font-size:.7rem; color:rgba(255,255,255,.4);">Directrice Marketing, TechHub Dakar</p>
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- ══════════════════════════════════════════════════════
         ...
         ══════════════════════════════════════════════════════ --}}
    <div style="background:var(--pearl); display:flex; flex-direction:column; justify-content:center; align-items:center; padding:2rem 1.5rem; min-height:100vh;">

        <div style="width:100%; max-width:26rem;">

            {{-- Logo mobile --}}
            <div class="lg:hidden animate-in" style="text-align:center; margin-bottom:2.5rem;">
                    ...
                    <div>
                        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:.5rem;">
                            ...
                            <a href="{{ route('password.request') }}"
                               style="font-size:.75rem; font-weight:600; color:var(--blue-electric); text-decoration:none; transition:opacity .2s;"
            ...
            <div class="animate-in delay-3" style="display:grid; grid-template-columns:1fr 1fr; gap:.75rem; margin-bottom:2rem;">
                ...
                <button type="button"
                        style="display:flex; align-items:center; justify-content:center; gap:.625rem; padding:.875rem; border-radius:1rem; border:1.5px solid var(--gray-soft); background:white; cursor:pointer; font-size:.82rem; font-weight:600; color:var(--blue-night); font-family:'Inter',sans-serif; transition:border-color .2s, box-shadow .2s;"
                        onmouseover="this.style.borderColor='var(--blue-electric)'; this.style.boxShadow='0 0 0 3px rgba(30,95,216,.08)'"
                        onmouseout="this.style.borderColor='var(--gray-soft)'; this.style.boxShadow='none'">
            ...
            <div class="animate-in delay-4" style="text-align:center; padding:1.25rem; border-radius:1.25rem; background:white; border:1.5px solid var(--gray-soft);">
                <p style="font-size:.82rem; color:var(--gray-mid);">
                    ...
                    <a href="{{ route('register') }}"
                       style="font-weight:700; color:var(--blue-electric); text-decoration:none; margin-left:.25rem; transition:opacity .2s;"
                       onmouseover="this.style.opacity='.75'" onmouseout="this.style.opacity='1'">
