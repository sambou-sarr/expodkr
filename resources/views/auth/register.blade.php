...
<div style="display:grid; min-height:100vh;"
    ...
    <div class="hidden lg:flex" style="background:var(--blue-night); position:relative; overflow:hidden; flex-direction:column; justify-content:space-between; padding:3rem;">
        ...
        {{-- Glows --}}
        <div style="position:absolute; top:-8rem; right:-8rem; width:28rem; height:28rem; border-radius:50%; background:var(--blue-electric); opacity:.07; filter:blur(80px);" aria-hidden="true"></div>
        <div style="position:absolute; bottom:-6rem; left:-6rem; width:22rem; height:22rem; border-radius:50%; background:var(--gold); opacity:.06; filter:blur(60px);" aria-hidden="true"></div>

        {{-- Logo --}}
        <div style="position:relative; z-index:1;">
            <a href="{{ route('home') }}" style="display:flex; align-items:center; gap:.75rem; text-decoration:none;">
                <div style="width:2.5rem; height:2.5rem; border-radius:.875rem; background:linear-gradient(135deg,var(--blue-electric),#1248b0); display:flex; align-items:center; justify-content:center; box-shadow:0 4px 16px rgba(30,95,216,.4);">
        ...
        <div style="position:relative; z-index:1;">
            <div style="display:inline-flex; align-items:center; gap:.625rem; padding:.5rem 1rem; border-radius:2rem; border:1px solid rgba(201,168,76,.3); background:rgba(201,168,76,.07); margin-bottom:2rem;">
                ...
                <span style="font-size:.7rem; font-weight:600; letter-spacing:.15em; text-transform:uppercase; color:var(--gold-light);">Inscription gratuite</span>
            </div>

            <h2 class="font-display" style="font-size:2.6rem; color:white; line-height:1.2; margin-bottom:1.25rem;">
                Rejoignez la<br>
                <span style="background:linear-gradient(135deg,var(--gold),var(--gold-light));-webkit-background-clip:text;-webkit-text-fill-color:transparent;">communauté</span><br>
                ExpoDKR
            </h2>

            <p style="font-size:.9rem; color:rgba(255,255,255,.55); line-height:1.7; max-width:28rem;">
                Accédez aux meilleurs événements professionnels du Sénégal, réservez vos billets et gérez vos participations en un seul endroit.
            </p>

           {{-- Avantages --}}
            <div style="display:flex; flex-direction:column; gap:.875rem; margin-top:2rem;">
                @foreach([
                    ['check', 'Inscription et accès 100% gratuits'],
                    ['ticket', 'Billets QR sécurisés par email'],
                    ['calendar', 'Agenda personnalisé de vos événements'],
                    ['lock', 'Données protégées et confidentielles'],
                ] as [$icon, $text])
                <div style="display:flex; align-items:center; gap:.875rem;">
                    <span style="display:inline-flex; align-items:center; justify-content:center; width:1.75rem; height:1.75rem; border-radius:50%; background:rgba(255,255,255,.08); flex-shrink:0;">
                        @switch($icon)
                            @case('check')
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.85)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                    <polyline points="20 6 9 17 4 12"></polyline>
                                </svg>
                                @break
                            @case('ticket')
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.85)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <span style="font-size:.85rem; color:rgba(255,255,255,.65);">{{ $text }}</span>
                </div>
                @endforeach
            </div>
            ...
            <div style="display:flex; gap:2rem; margin-top:2.5rem; padding-top:2rem; border-top:1px solid rgba(255,255,255,.08);">
                @foreach([['+15k','Membres'],['240+','Événements'],['98%','Satisfaction']] as [$val,$lbl])
                <div>
        ...
        <div style="position:relative; z-index:1; padding:1.25rem; border-radius:1.25rem; background:rgba(255,255,255,.05); border:1px solid rgba(255,255,255,.08);">
            <div style="display:flex; gap:.25rem; margin-bottom:.75rem;">
                @for($i=0;$i<5;$i++)<svg style="width:.875rem;height:.875rem;color:var(--gold);" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>@endfor
            </div>
            <p style="font-size:.82rem; color:rgba(255,255,255,.65); line-height:1.6; font-style:italic;">
                "J'ai découvert ExpoDKR avant le Forum Invest Dakar. L'inscription était super simple et mon billet était dans ma boîte en moins d'une minute."
            </p>
            <div style="display:flex; align-items:center; gap:.75rem; margin-top:.875rem;">
                <div style="width:2rem; height:2rem; border-radius:50%; background:linear-gradient(135deg,var(--blue-electric),var(--gold)); display:flex; align-items:center; justify-content:center; font-size:.75rem; font-weight:700; color:white;">M</div>
                <div>
                    <p style="font-size:.78rem; font-weight:600; color:white;">Moussa Ndiaye</p>
                    <p style="font-size:.7rem; color:rgba(255,255,255,.4);">Entrepreneur, Dakar</p>
                </div>
            </div>
        </div>
    </div>
